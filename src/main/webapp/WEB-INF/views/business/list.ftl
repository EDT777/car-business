                ...
                <div style="margin: 20px 0px 0px 10px">
                    <form class="form-inline" id="searchForm" action="/business/list" method="post">
                        <input type="hidden" name="currentPage" id="currentPage" value="1">
                                ...
                                <div class="form-group">
                                    <label>经营日期查询：</label>
                                    <input placeholder="请输入开始日期" name="startOpenDate" type="text" class="form-control input-date" value="${(qo.startOpenDate?string('yyyy-MM-dd'))!}" /> -
                                    <input placeholder="请输入结束日期" name="endOpenDate" type="text" class="form-control input-date"  value="${(qo.endOpenDate?string('yyyy-MM-dd'))!}"/>
                                </div>
                ...
                <table class="table table-hover table-bordered table-striped">
                    ...
                    <#list pageInfo.list as b>
                        <tr>
                            ...
                            <td>${b.tel}</td>
                            <td>${b.address}</td>
                            <td>${(b.openDate?string('yyyy-MM-dd'))!}</td>
                            <td>${(b.mainStore?string('总店','分店'))!}</td>
                            <td>
                                <a href="/business/input?id=${b.id}" class="btn btn-info btn-xs">
